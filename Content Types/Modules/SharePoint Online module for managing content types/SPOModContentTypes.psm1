...
function New-SPOListContentType
{
    ...
    switch ($PsCmdlet.ParameterSetName)
    {
        ...
        "setb" {
            $lci.ParentContentType = $ctx.Web.ContentTypes.GetById($ParentContentTypeID)
            $ContentType = $ctx.Web.Lists.GetByTitle($ListName).ContentTypes.Add($lci)
            break
        }
        "setc" {
    ...
    {
        $ctx.ExecuteQuery()
        Write-Host "Content Type `"$Name`" has been added to the list" -ForegroundColor Green
    }
...
function New-SPOSiteContentType
{
    ...
    {
        $ctx.ExecuteQuery()
        Write-Host "Content Type `"$Name`" has been added to the site" -ForegroundColor Green
    }
    catch [Net.WebException]
    {
        Write-Host $_.Exception.ToString() -ForegroundColor Red
...
function Set-SPOContentType
{
    ...
    if ($PSBoundParameters.ContainsKey("Group")) { $ctType.Group = $Group }
    if ($PSBoundParameters.ContainsKey("DisplayFormUrl")) { $ctType.DisplayFormUrl = $DisplayFormUrl }
    if ($PSBoundParameters.ContainsKey("EditFormUrl")) { $ctType.EditFormUrl = $EditFormUrl }
    if ($PSBoundParameters.ContainsKey("Hidden")) { $ctType.Hidden = $Hidden }
    if ($PSBoundParameters.ContainsKey("JSLink")) { $ctType.JSLink = $JSLink }
    if ($PSBoundParameters.ContainsKey("NewFormUrl")) { $ctType.NewFormUrl = $NewFormUrl }
    if ($PSBoundParameters.ContainsKey("ReadOnly")) { $ctType.ReadOnly = $ReadOnly }
    if ($PSBoundParameters.ContainsKey("Sealed")) { $ctType.Sealed = $Sealed }
